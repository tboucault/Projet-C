...
void main (void)
 {
     char tab [100];
     char format[3];
     int i=0,j=0,k=0,l=0,m=0,n=0,ilongueur=0,ilargeur=0;
     char c;
     char longueur[3];
     char largeur[3];
     char contenu[10];
     ...
     if (( Image_Origine != NULL )&&( Image_Nouvelle != NULL ))
        {
            ...
            if(ispgm(Image_Origine,Image_Nouvelle)==1) // si fichier pgm, on continu
            {
                ...
                /* trouver une methode pour copier le fichier selon longueur et largeur prédéfini (dynamique)
                sscanf(longueur,"%d",&ilongueur);
                sscanf(largeur,"%d",&ilargeur);*/
                // demande du seuil à l'utilisateur
                printf("Valeur du seuil: \n");
                scanf("%d",&seuil);

                for (n=0; n<(512*512); n++)
                {
                    fgets(tab ,100 ,Image_Origine );//extraction de la valeur d'un pixel du fichier
                    sscanf(tab,"%s",&pixel);//stockage de la valeur dans le tableau pixel[]
                    if(atoi(pixel)<seuil)
                    {
                        sprintf(pixel,"%s","0\n");
                        fputs(pixel,Image_Nouvelle );
                    }
                    else if(atoi(pixel)>=seuil)
                    {
                        sprintf(pixel,"%s","255\n");
                        fputs(pixel,Image_Nouvelle );
                    }
                    else
                        printf("Erreur comparaison seuil\n"); //message d'erreur
                }
                printf("   - Duplication de l'image: OK\n");
               ...
               /* for (n=0; n<(512*512); n++)
                ...
                printf("   - Copie dans le fichier de destination: OK\n");
